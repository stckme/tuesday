...
def total_comments(since=None):
    if not since:
        since, = (first_comment_created for first_comment_created, in Comment.select(
            Comment.created).order_by(Comment.created).limit(1).tuples())
    approved, = (count for count, in Comment.select(
        fn.count(Comment.id)).where(Comment.created >= since).tuples())
    rejected, = (count for count, in RejectedComment.select(
        fn.count(RejectedComment.id)).where(RejectedComment.created >= since).tuples())
...
def monthly_comments_count_lastNmonths(n=4):
    return monthly_comments_count(arrow.utcnow().shift(months=-int(n)).span('day')[0].date())
...
def weekly_comments_count_lastNweeks(n=4):
    return weekly_comments_count(arrow.utcnow().shift(weeks=-int(n)).span('day')[0].date())
...
def pending_comments_by_asset():
    pending_comments_by_asset = PendingComment.select(PendingComment.asset_id, fn.count(
        PendingComment.id)).group_by(PendingComment.asset_id).tuples()
    total_pending = sum(
...
def curr_week_topN_commenters(n=3):
    curr_week_top10_commenters = Comment.select(fn.date_trunc(
        'week', Comment.created), Comment.commenter_id, fn.count(Comment.id)).group_by(fn.date_trunc(
            'week', Comment.created), Comment.commenter_id).where(Comment.created >= arrow.utcnow().span('week')[0].date()).order_by((SQL('count').desc())).limit(int(n)).tuples()
...
def curr_month_topN_commenters(n=3):
    curr_month_top10_commenters = Comment.select(fn.date_trunc(
        'month', Comment.created), Comment.commenter_id, fn.count(Comment.id)).group_by(fn.date_trunc(
            'month', Comment.created), Comment.commenter_id).where(Comment.created >= arrow.utcnow().span('month')[0].date()).order_by((SQL('count').desc())).limit(int(n)).tuples()
...
def curr_year_topN_commenters(n=3):
    curr_year_top10_commenters = Comment.select(fn.date_trunc(
        'year', Comment.created), Comment.commenter_id, fn.count(Comment.id)).group_by(fn.date_trunc(
            'year', Comment.created), Comment.commenter_id).where(Comment.created >= arrow.utcnow().span('year')[0].date()).order_by((SQL('count').desc())).limit(int(n)).tuples()
...
def monthly_topN_commenters(n=3):
    ...
    def output_formatter(mtc): return (mtc[0].date().strftime(
        "%B %Y"), Member.get_by_id(mtc[1]).name, mtc[2])
    for month in months:
        monthly_top_commenters.extend(list(map(output_formatter, Comment.select(fn.date_trunc(
            'month', Comment.created), Comment.commenter_id, fn.count(Comment.id)).group_by(fn.date_trunc(
                'month', Comment.created), Comment.commenter_id).where(fn.date_trunc(
                    'month', Comment.created) == month).order_by((SQL('date_trunc')), (SQL('count')).desc()).limit(int(n)).tuples())))
...
def monthly_topN_commented_articles(n=10):
    ...
    def output_formatter(mtc): return (mtc[0].date().strftime(
        "%B %Y"), Asset.get_by_id(mtc[1]).url, mtc[2])
    for month in months:
...
def last2days_topN_commented_articles(n=10):
    top10_commented_articles_last2days = Comment.select(Comment.asset_id, fn.count(Comment.id)).group_by(Comment.asset_id).where(
        Comment.created >= arrow.utcnow().shift(days=-int(2)).span('day')[0].date()).order_by((SQL('count')).desc()).limit(int(n)).tuples()
...
def get_all_stats():
    def is_function_local(object): return isinstance(
        object, types.FunctionType) and object.__module__ == __name__
    data = {stat: getattr(sys.modules[__name__], stat)() for stat, stat_data in inspect.getmembers(
        sys.modules[__name__], predicate=is_function_local) if stat != 'get_all_stats'}
    return data
